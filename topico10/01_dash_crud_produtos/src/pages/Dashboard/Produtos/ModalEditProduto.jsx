...
const ModalEditProduto = ({ close, editedProduto }) => {
  const {editProduto} = useProdutosContext()
  ...
  useEffect(() => {
    ...
    loadFornecedores()
    // .then(()=>{
    //     listFornecedores.each(fornecedor=>{
    //       if(fornecedor.id === editProduto.fornecedor_id)
    //           selectedFornecedorId.current.value = fornecedor.id;
    //     })
    // })
  }, [])
            ...
            >
              {listFornecedores.map((fornecedor, i) => (
                <option 
                  key={i} 
                  value={fornecedor.id} 
                  selected={fornecedor.id===editedProduto.fornecedor_id}
